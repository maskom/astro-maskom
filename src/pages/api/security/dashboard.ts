...
export const GET: APIRoute = async ({ request, cookies, url }) => {
  try {
    ...
    const hasPermission = await rbacService.hasPermission(securityContext.userId, Permission.VIEW_SECURITY_LOGS);
    ...
    // Get security events
    const events = await securityAuditLogger.getSecurityEvents(userId, severity, limit);
    
    // Get audit logs
    const auditLogs = await securityAuditLogger.getAuditLogs(userId, undefined, limit);
    ...
    if (await rbacService.hasRole(securityContext.userId, 'admin' as any)) {
      ...
      userSessions = await sessionManager.getUserActiveSessions(securityContext.userId);
    }
...
async function getSecurityStats(userId: string) {
  try {
    ...
    const recentEvents = await securityAuditLogger.getSecurityEvents(undefined, undefined, 1000);
    ...
    const recentLogs = await securityAuditLogger.getAuditLogs(userId, undefined, 1000);
    ...
    const suspiciousSessions = await sessionManager.detectSuspiciousSessions(userId);
    ...
    return {
      ...
      lastLogin: recentLogs.find(log => log.action === 'login' && log.success)?.timestamp || null,
      mfaEnabled: await (await import('../../../lib/security/mfa')).mfaService.isMFAEnabled(userId)
    };
  } catch (error) {
