...
import { Permission, UserRole, SecurityAction } from "../../../lib/security/types";
...
export const GET: APIRoute = async ({ request, cookies, url }) => {
  try {
    ...
    if (targetUserId && targetUserId !== securityContext.userId) {
      const hasPermission = await rbacService.hasPermission(securityContext.userId, Permission.VIEW_CUSTOMERS);
    ...
    // Get data consents
    const { data: consents } = await (await import('@supabase/supabase-js')).createClient(
...
export const PUT: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (targetUserId !== securityContext.userId) {
      const hasPermission = await rbacService.hasPermission(securityContext.userId, Permission.MANAGE_CUSTOMERS);
    ...
    if (role && Object.values(UserRole).includes(role)) {
      const roleSuccess = await rbacService.assignRole(targetUserId, role, securityContext.userId);
    ...
    if (permissions && Array.isArray(permissions)) {
      ...
      const currentPermissions = await rbacService.getUserPermissions(targetUserId);
      
      // Grant new permissions
      for (const permission of permissions) {
        if (!currentPermissions.includes(permission as Permission)) {
          const grantSuccess = await rbacService.grantPermission(targetUserId, permission as Permission, securityContext.userId);
      ...
      for (const permission of currentPermissions) {
        if (!permissions.includes(permission)) {
          const revokeSuccess = await rbacService.revokePermission(targetUserId, permission, securityContext.userId);
...
export const DELETE: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (!targetUserId || !confirm) {
      return new Response("Target user ID and confirmation are required", { status: 400 });
    }

    // Only super admins can delete users
    const isSuperAdmin = await rbacService.hasRole(securityContext.userId, UserRole.SUPER_ADMIN);
    ...
    await (await import('../../../lib/security/session')).sessionManager.invalidateAllUserSessions(targetUserId);
