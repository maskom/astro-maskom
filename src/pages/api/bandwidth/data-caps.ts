...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    if (capsError && capsError.code !== 'PGRST116') { // Not found error
    ...
    const { data: history, error: historyError } = await supabase
      ...
      .select('*')
      .eq('user_id', user.id)
      .gte('month', new Date(Date.now() - 6 * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0].substring(0, 7))
    ...
    return new Response(JSON.stringify({
      ...
      history: history || [],
      summary: dataCaps ? {
        usagePercentage: (dataCaps.current_usage_gb / dataCaps.monthly_cap_gb) * 100,
        remainingGB: dataCaps.monthly_cap_gb - dataCaps.current_usage_gb,
        daysRemaining: Math.ceil((new Date(dataCaps.billing_cycle_start).getTime() + 30 * 24 * 60 * 60 * 1000 - Date.now()) / (24 * 60 * 60 * 1000))
      } : null
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!package_id || !monthly_cap_gb) {
      return new Response(JSON.stringify({ error: 'Package ID and monthly cap required' }), { 
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }
...
export const PUT: APIRoute = async ({ request }) => {
  try {
    ...
    if (notification_thresholds) updateData.notification_thresholds = notification_thresholds;
