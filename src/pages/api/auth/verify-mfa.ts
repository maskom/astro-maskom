...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (!code && !backupCode) {
      return new Response("Verification code or backup code is required", { status: 400 });
    }
    ...
    if (code) {
      ...
      // Verify backup code
      isValid = await mfaService.verifyBackupCode(securityContext.userId, backupCode);
    ...
    const sessionUpdated = await sessionManager.verifyMFAForSession(securityContext.sessionId);
