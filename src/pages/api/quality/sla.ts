...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const slaData = await qualityService.getSLACompliance(user.id, months);
    
    return new Response(JSON.stringify({ slaData }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('SLA compliance API error:', error);
