...
export class SessionManager {
  ...
  async invalidateAllUserSessions(
    ...
    try {
      ...
      logger.error(
        'User sessions invalidation error',
        error instanceof Error ? error : new Error(String(error)),
        {
          userId,
          exceptSessionId,
  ...
  async verifyMFAForSession(sessionId: string): Promise<boolean> {
    try {
      ...
      if (error) {
        logger.error(
          'Failed to verify MFA for session',
          error instanceof Error ? error : new Error(String(error)),
          {
            sessionId,
            module: 'session',
      ...
      logger.error(
        'MFA session verification error',
        error instanceof Error ? error : new Error(String(error)),
        {
          sessionId,
          module: 'session',
  ...
  async detectSuspiciousSessions(userId: string): Promise<SessionSecurity[]> {
    try {
      ...
      logger.error(
        'Suspicious session detection error',
        error instanceof Error ? error : new Error(String(error)),
        {
          userId,
          module: 'session',
          operation: 'detectSuspicious',
        }
      );
      return [];
    }
  ...
  requireMFAVerification() {
    return async (sessionId: string): Promise<boolean> => {
      try {
        ...
        logger.error(
          'MFA verification check error',
          error instanceof Error ? error : new Error(String(error)),
          {
            sessionId,
            module: 'session',
            operation: 'checkMFAVerification',
          }
        );
        return false;
      }
